...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!["✩"].some(ss => member.user.username.toLowerCase().includes(ss)) && member.user.discriminator !== "etiket varsa koyun yoksa 0000 yapın örnek : 0099 şeklinde yazın" && !message.guild.members.cache.get(member.id).roles.cache.has('835544857584992294' && '835544857584992294')) {
            return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: false })).setDescription("Kullanıcının kayıt olabilmesi için boost basmalı veya tag almalı!").setTimestamp().setThumbnail(message.author.avatarURL).setFooter('Vip almak için : .vip @kullanıcı/id'))
        }
        message.guild.members.cache.get(member.id).setNickname(`✩ ${name} | ${age}`)
        db.push(`isimler_${member.id}`, ` \`${name} | ${age}\` (<@&835544857594429451>)`);
        ...
        message.channel.send(embed2.setDescription(`${member} **adlı kullanıcı \`Erkek\` Olarak Kayıt Edildi!**`)
.setFooter('✩ Sword')
